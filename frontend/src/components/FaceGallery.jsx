...
export default function FaceGallery({ faces = [], currentTime, onSeekToTimestamp, onClearFaces }) {
  ...
  const filteredFaces = useMemo(() => {
    return uniqueFaces.filter((f) => {
      if (filterType === 'BREACHES' && !f.in_restricted_zone && !f.is_running) return false;
      if (filterType === 'NORMAL' && (f.in_restricted_zone || f.is_running)) return false;
      if (searchQuery.trim()) {
      ...
      <div className='px-5 py-3 border-b border-slate-800/60 bg-slate-900/40 flex flex-wrap items-center justify-between gap-3'>
        <div className="flex items-center gap-1.5">
          {[
            ...
            { id: "BREACHES", label: "Breaches / Running" },
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1 space-y-1">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {!face.in_restricted_zone && !face.is_running && (
